...
<html lang="en">
  <head>
    ...
    <title>Zshop商城-管理-管理员列表-第<?php echo ($currentPage); ?>页</title>
    <!-- Bootstrap core CSS -->
                  ...
                  <div class="col-lg-12">
                      <section class="panel">
                          <header class="panel-heading">
                              组列表
                              <a href="/Admin/Auth/add"><button type="button" class="btn btn-success pull-right btn-sm">添加权限组</button></a>    
                          </header>
                          <table class="table table-striped table-advance table-hover">
                              <thead>
                              <tr>
                                  <th><i class="icon-bullhorn"></i> 权限组ID</th>
                                  <th class="hidden-phone"><i class="icon-question-sign"></i> 权限组名称</th>
                                  <th>操作</th>
                              </tr>
                              </thead>
                              <tbody>
                              <?php if(!empty($groupList) and is_array($groupList)): if(is_array($groupList)): $i = 0; $__LIST__ = $groupList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$groupList): $mod = ($i % 2 );++$i;?><tr>
                                  <td><?php echo ($groupList["id"]); ?></td>
                                  <td class="hidden-phone"><?php echo ($groupList["title"]); ?></td>
                                  <td>
                                  <a class="btn btn-primary btn-xs" href="/Admin/Auth/groupAccessList/groupId/<?php echo ($groupList["id"]); ?>">查看组权限</a>&nbsp;|&nbsp;
                                  <a class="btn btn-primary btn-xs" href="/Admin/Auth/updates/id/<?php echo ($groupList["id"]); ?>">修改</a>&nbsp;|&nbsp;
                                  <a class="btn btn-xs btn-warning myModal2" data-toggle="modal" href="#myModal2" value="<?php echo ($groupList["id"]); ?>">删除</a>
                                  </td>
            ...
            $("#confirmvalue").click(function(){
              ...
              $("body").removeClass("modal-open");
                var roleId = $(this).attr("value");
                  $.post("/Admin/Auth/doDelete",{id:roleId},function(data,textStatus, jqXHR){
