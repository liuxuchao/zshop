...
class Mongo extends Driver {
    ...
    public function update($data,$options) {
        ...
        try{
            ...
            //数据不存在时支持插入 董光明 2017-01-06
            if(isset($options['upsert']) && $options['upsert']) {
                $multiple   =   array("upsert" => true);
            }else{
                $multiple   =   array("upsert" => false);
            }
     ...
     *   }
     */
    public function upsert($criteria, $newData, $options)
    {
        if(isset($options['table'])) {
            $this->switchCollection($options['table']);
        }
        $result   = $this->_collection->update($criteria, $newData, $options);
        return $result;
    }
