...
class Date {
    ...
    /**
     * *
     * 转时间格式今天 ,昨天,10-24 ,2015-10-24
     * @AuthorHTL 郑自强<zhengziqiang@qiaodata.com>
     * @DateTime  2016-10-24T17:50:18+0800
     * @param     int  $time 时间戳
     * @return    string | bool
     */
    public function forMatTime($time)
    {
        if($time <=0 || $this->date <=0){
            return false ;
        }
        if(date('Y',$time) != date('Y',$this->date) && date('Y-m-d',$time) != date('Y-m-d',$this->date) && date('Y-m',$time) != date('Y-m',$this->date)){
            return date('Y-m-d',$this->date);
        }elseif(date('Y-m-d',$time) == date('Y-m-d',$this->date)){
            return '今天';
        }elseif(date('Y-m',$time) == date('Y-m',$this->date) && (date('Ymd',$time) - date('Ymd',$this->date)) == 1 ){
            return '昨天';
        }elseif(date('Y',$time) == date('Y',$this->date) && (date('Ymd',$time) - date('Ymd',$this->date)) > 1 ){
     ...
     * [更精准的计算时间差,精确到月]
     * @AuthorHTL 郑自强<zhengziqiang@qiaodata.com>
     * @DateTime  2016-11-28T13:32:24+0800
    ...
    public function timeDifference($startTime='',$endTime=''){
        ...
        if(empty($endTime)){
            $endTime=date('Y-m-d',$_SERVER['REQUEST']);
        }
        $stimeData = explode('-', str_replace('.', '-', $startTime));
        $etimeData = explode('-', str_replace('.', '-', $endTime));
        if( 0 > ($etimeData['1'] - $stimeData['1'])){
            $month = ($etimeData['1']+12 - $stimeData['1']);
            $year  =  (($etimeData['0'] - $stimeData['0'])-1) ;
        }else{
            $month = ($etimeData['1'] - $stimeData['1']) ;
            $year  =  ($etimeData['0'] - $stimeData['0']) ;
        }
        $work_time = '';
        if( 0 !=  $year ){
            $work_time .= $year.'年';
        ...
        $work_time = $work_time?$work_time:'1个月';
        return $work_time ;
    }
